...
int main(int argc, char *argv[]) try
{
	...
	{
		source = "testing.png";
		message = "restored.dat";
		original = "carrier.png";
		pass = "password";


	}
	Mat src = imread( original, 1 ); //Read original source image
	Mat src_message = imread( source, 1 ); //Matrix to Store image with hidden message
	Mat src_hidden_message = Mat::zeros( src.size(), src.type() ); // Matrix to store output image with information
	...
	get_hidden_message(src, src_message, src_hidden_message); // Restore randomized hidden message from image
	Mat src_hidden_message_dereorderd = Mat::zeros( src.size(), src.type() ); // Matrix to store readed hidden message
	de_reorder_color(src_hidden_message, src_hidden_message_dereorderd, rand_mat); // de-randomize message
	mat2bin(message,src_hidden_message_dereorderd); // save message into file
